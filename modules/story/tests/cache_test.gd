...
func _initialize() -> void:
	...
	write(path, source)
	var aliases := {"user://chapter.one.en.md": "chapter.one"}
	var cache := StoryProgramCache.new()
	var first := cache.compile_file(path, "start", aliases)
	first.runtime.set("count", 99)
	first.runtime.get("items").append(2)
	first.instructions[0].data.content = "Mutated"
	var second := cache.compile_file(path, "start", aliases)
	...
	var changed := cache.compile_file(path, "start", aliases)
	...
	aliases["user://chapter.one.en.md"] = "renamed"
	var mapped := cache.compile_file(path, "start", aliases)
	check(mapped.instructions[1].data.story == "renamed" and cache.compilations == 3, "alias mapping invalidates compiled jump")
	cache.compiler_version = "future-test-version"
	var recompiled := cache.compile_file(path, "start", aliases)
	check(cache.compilations == 4 and recompiled.runtime_script != mapped.runtime_script, "compiler version invalidates")
	cache.capacity = 1
	cache.compile_file(path, "another", aliases)
	check(cache.size() == 1, "cache shrinks to bounded capacity")
	var before := cache.compilations
	cache.compile_file(path, "start", aliases)
	...
	cache.compile_file(path, "start", aliases, false)
	cache.compile_file(path, "start", aliases, false)
