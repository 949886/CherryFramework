...
- 右侧复用 Godot 原生 `CodeEdit`，支持行号、缩略图、原生剪贴板及独立撤销历史。Story 高亮区分标题、说话人、状态、命令、参数、跳转路径、粗体和注释；`gdscript` 围栏及独立反引号代码行复用 Godot 原生 `GDScriptSyntaxHighlighter`，支持关键字、类型、函数、字符串与代码注释。正文和语法颜色使用 Story 专属的明亮淡紫色配色，集中配置在 `editor/story_syntax_palette.tres` 的 `StorySyntax` 颜色项中，修改后即时更新；`text` 控制对白、旁白、选项和代码普通文本的颜色，默认使用不透明的浅白色。GDScript 保留原生词法识别，仅替换输出颜色；背景、字体和字号继续跟随编辑器设置，不修改 Godot 全局配色。高亮不会执行剧情代码，也不提供 GDScript 调试器或完整语义补全。
